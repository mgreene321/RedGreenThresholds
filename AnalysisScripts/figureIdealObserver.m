...
hsvMatrix_Red1 = zeros(numColorMapRows,3);
hsvMatrix_Red1(:,1) = 0; % red hue
hsvMatrix_Red1(:,2) =1;%pixVals.^-0.07; % vary saturation
hsvMatrix_Red1(:,3) = 0.7.*(pixVals.^(1/8) - 1) + 0.3.*linspace(0,1, numColorMapRows);%pinkColorMapHSV(:,3);%linspace(0.75,1, 256); %linspace(0.25,1,256); % keep "value" constant
rgbMatrix_Red1 = hsv2rgb(hsvMatrix_Red1);

redRGB1 = @(x) interp1(linspace(0,1,256), rgbMatrix_Red1, x);

hsvMatrix_Red2 = zeros(numColorMapRows,3);
hsvMatrix_Red2(:,1) = 0; % red hue
hsvMatrix_Red2(:,2) = 0.9.*(pixVals/256)+0.1;%pixVals.^-0.07; % vary saturation
hsvMatrix_Red2(:,3) = 0.7.*(pixVals.^(1/8) - 1) + 0.3.*linspace(0,1, numColorMapRows);%pinkColorMapHSV(:,3);%linspace(0.75,1, 256); %linspace(0.25,1,256); % keep "value" constant
rgbMatrix_Red2 = hsv2rgb(hsvMatrix_Red2);

redRGB2 = @(x) interp1(linspace(0,1,256), rgbMatrix_Red2, x);
...
figure;
imagesc(imcrop(PSF(:,:,wvl==680)./(max(max(PSF(:,:,(wvl==680))))), cropRect)), colormap(rgbMatrix_Red1), axis equal off
...
figure, imagesc(imcrop(coneAperturesNorm(:,:,1:3), cropRect)); hold on; alpha(0.6); axis off; axis equal;

% plot(xx{1}, yy{1}, 'LineWidth', 3, 'Color', hsv2rgb([redHue 1 0.33]));
% plot(xx{2}, yy{2}, 'LineWidth', 3, 'Color', hsv2rgb([redHue 1 0.67]));
% plot(xx{3}, yy{3}, 'LineWidth', 3, 'Color', hsv2rgb([redHue 1 1]));
% % 
plot(xx{1}, yy{1}, 'LineWidth', 4, 'Color', redRGB1(0.1));
plot(xx{2}, yy{2}, 'LineWidth', 4, 'Color', redRGB1(0.6));
plot(xx{3}, yy{3}, 'LineWidth', 4, 'Color', redRGB1(1));
...
total680Isom = (10^11.4).*stim680Isom + bkgIsom;
isoms = zeros(712);
...
figure; imagesc(canvas), colormap gray, hold on; axis off equal;

for i =1:numel(a)
    %plot(a(i), b(i), 'Marker', 'o', 'MarkerEdgeColor', 'none', 'MarkerFaceColor', isoms(b(i), a(i)).*ones(1,3), 'MarkerSize', 20)
    plot(a(i), b(i), 'Marker', 'o', 'MarkerEdgeColor', 'none', 'MarkerFaceColor', redRGB2(isoms(b(i), a(i))), 'MarkerSize', 20)
end
% colormap(rgbMatrix_Red2(6,:))
colormap([0 0 0]);
