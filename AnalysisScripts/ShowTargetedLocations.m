%plot targeted stimulus locations for subject
...
% maxRGratio = max(subjectAnalyzedData.GreenThreshold_J./subjectAnalyzedData.RedThreshold_J);
% minRGratio = min(subjectAnalyzedData.GreenThreshold_J./subjectAnalyzedData.RedThreshold_J);
...
figure, imagesc(OCTimage),hold on, colormap gray, axis equal off
...
xm_mean = mean(xm); ym_mean = mean(ym);

%rectColor = [(RGratio - minRGratio)./(maxRGratio - minRGratio) 1-(RGratio - minRGratio)./(maxRGratio - minRGratio) 0];
rectColor = 'w';
 drawrectangle('Position', [xm_mean-11 ym_mean-11 21 21], 'LineWidth', 1.5, 'Color', rectColor, 'FaceAlpha', 0.3, 'InteractionsAllowed', 'none');
...
% Plot stimulus on average of cross locations
end