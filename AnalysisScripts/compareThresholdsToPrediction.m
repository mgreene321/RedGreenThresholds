...
scanAngleDeg = 0.9;
scanAngleUm = DegreesToRetinalMM(scanAngleDeg, eyeLengthMm) * 1e3;
    ...
    for f = 1:length(folders)
        ...
        for b = 1:length(blocks)
            ...
            for i = 1:length(analysisOutput.greenThresholdPowerAU)
                ...
                [~, tRedRU] = convertAOArbitraryToRealUnits(tRedAU,redPowerWattsAt680nm,680,redFWHM,scanAngleDeg, photoreceptorsBleached);
                [~, tGreenRU] = convertAOArbitraryToRealUnits(tGreenAU,greenPowerWattsAt543nm,543, greenFWHM,scanAngleDeg, photoreceptorsBleached);
                
                % quanta incident at the cornea
                incidentRedQuanta{s}(locCounter,:) = sum(tRedRU.rawCornIrradianceQuantaPerLambdaUm2SecIn) .* stimRenderTimeSec .* pupilAreaMm2 .* 1e6;
                incidentGreenQuanta{s}(locCounter,:) = sum(tGreenRU.rawCornIrradianceQuantaPerLambdaUm2SecIn) * stimRenderTimeSec  * pupilAreaMm2 .* 1e6;
                
                
                % total quanta incident at photoreceptors during full stimulus presentation, accounting for
                % pre-retinal filtering 
                
                transmittedRedQuanta{s}(locCounter,:) = sum(tRedRU.transmittedQuantaPerLambdaUm2Sec) .* stimRenderTimeSec .* scanAngleUm^2; %pupilAreaMm2 .* 1e6;
                transmittedGreenQuanta{s}(locCounter,:) = sum(tGreenRU.transmittedQuantaPerLambdaUm2Sec) .* stimRenderTimeSec .* scanAngleUm^2; %pupilAreaMm2 .* 1e6;
                ...
                gMax{s}(locCounter,:) = max(tGreenRU.rawCornIrradianceQuantaPerLambdaUm2SecIn(:));
                rMax{s}(locCounter,:) = max(tRedRU.rawCornIrradianceQuantaPerLambdaUm2SecIn(:));
