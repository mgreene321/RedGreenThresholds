function [coneApertures, cone_locs, cone_labels] = dailyConeApertures(subjectId, expDate)
...
tempDir = dir(fullfile(root, 'Documents', '**', '*RedGreenThresholds', subjectId, expDate, 'TransferFromOCT'));
...
retina_map = imread(fullfile(transferFullPath,'meanFrame.tif'));

load(fullfile(transferFullPath, 'dailyData.mat'));
cone_locs = str2double(dailyData(:, [1 2]));
cone_labels = dailyData(:,end);
...
X_apertures = zeros(size(retina_map));
...
%                 S_apertures = max(cat(3, S_apertures, S_temp),[],3);
                  X_temp = zeros(size(retina_map));
                  X_temp(Yc-fix(windowSize/2):Yc+fix(windowSize/2), Xc-fix(windowSize/2):Xc+fix(windowSize/2)) = coneAperture;
                  X_apertures = max(cat(3, X_apertures, X_temp),[],3);
